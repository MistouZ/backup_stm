...
$count = $bd->query('SELECT * FROM factures  WHERE societe !="nmcp" AND societe !="hydro" AND societe !="si2p" AND etat != "P" GROUP BY num_fact ORDER BY num_fact ASC ');
$maxrow = $count->rowCount();
$reponse = $bd->query('SELECT * FROM factures WHERE societe !="nmcp" AND societe !="hydro" AND societe !="si2p" AND etat != "P" GROUP BY num_fact ORDER BY num_fact ASC  LIMIT '.$row.',1');
