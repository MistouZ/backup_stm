...
$count = $bd->query('SELECT * FROM factures  WHERE `annee` >= 2018  AND societe !="nmcp" AND societe !="hydro" AND societe != "databack" AND etat != "P" GROUP BY num_fact ORDER BY num_fact ASC ');
$maxrow = $count->rowCount();
$reponse = $bd->query('SELECT * FROM factures WHERE `annee` >= 2018 AND societe !="nmcp" AND societe !="hydro" AND societe != "databack" AND etat != "P" GROUP BY num_fact ORDER BY num_fact ASC  LIMIT '.$row.',1');
...
{
    echo "je passe là";
    header('Location: http://test.bitwin.nc/backup_missing_folder_invoice.php?folderId='.$donnees["dossier"].'&invoice='.$row);
}
    ...
    <div class="col-md-12">
        ...
        <div class="portlet box blue-chambray">
            <div class="portlet-title">
                <div class="caption">
                    <i class="fas fa-file-medical"></i>Modification du devis <span style="font-weight: 800; font-style: italic;"><?php echo $donnees["num_fact"]; ?></span></div>
            </div>
            <div class="portlet-body form">
                <!-- BEGIN FORM-->
                <form action="<?php echo URLHOST."_pages/_post/creer_facture_backup.php"; ?>" method="post" id="facture" name="facture" class="form-horizontal">
                    <div class="form-body">
                        ...
                        <div>
                            ...
                            <input type="hidden" name="quotationNumber" value="<?php echo $donnees["num_fact"];?>" />
