...
$count = $bd->query('SELECT * FROM dossier  WHERE societe !="nmcp" AND societe !="hydro" AND `annee` >= 2022 AND client != "" GROUP BY client');
$maxrow = $count->rowCount();

$req = $bd->query('SELECT * FROM dossier  WHERE societe !="nmcp" AND societe !="hydro" AND `annee` >= 2022 AND client != "" GROUP BY client LIMIT '.$i.',1 ');
$recup = $req->fetch();
